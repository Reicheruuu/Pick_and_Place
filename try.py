...
class MainWindow(QDialog):
    ...
    def detect_yellow(self):
        ...
        if ret:
            ...
            y_lower2 = np.array([190, 100, 100])
            y_upper2 = np.array([200, 255, 255])
            # Create a mask for yellow color
            y_mask = cv2.inRange(hsv_frame, y_lower1, y_upper1) + cv2.inRange(hsv_frame, y_lower2, y_upper2)
    ...
    def detect_green(self):
        ...
        if ret:
            ...
            g_lower2 = np.array([210, 40, 40])
            g_upper2 = np.array([256, 255, 255])
            # Create a mask for green color
            g_mask = cv2.inRange(hsv_frame, g_lower1, g_upper1) + cv2.inRange(hsv_frame, g_lower2, g_upper2)
